...
//  WordScramble
//
//  Created by Lorenzo Pesci on 16/07/22.
//
...
struct ContentView: View {
            ...
            List {
                Section {
                    TextField("Enter yout word", text: $newWord)
                        .autocapitalization(.none) // lowercase by default
                }
    ...
    func addNewWord() {
        ...
        guard isReal(word: answer) else {
            wordError(title: "Word not recognized", message: "You can't just male them up, you know!")
            return
        }
        
        withAnimation {
            usedWords.insert(answer, at: 0) // INSERT WORD AT THE TOP OF THE LIST
        }
        newWord = ""
        score += answer.count
    }
    ...
    func startGame() {
        ...
        if let startWordsURL = Bundle.main.url(forResource: "start", withExtension: "txt") {
            if let startWords = try? String(contentsOf: startWordsURL) {
                let allWords = startWords.components(separatedBy: "\n") //words separated by space line in start.txt file
